...
<header>
  ...
  <div class="container">
      <section>
        ...
        <?php
        $numbers_array = $_GET['digits'];
        $total = 0 ;
        $totalValidNumbers = (array) null;
        $invalid_values = (array) null;

        foreach ($numbers_array as $value){
          if(is_numeric($value)){
            $totalValidNumbers[] = $value;
            $total += $value;
          } else {
        ...
        print("The maximum value entered is: " . max($totalValidNumbers) . "</br>");
        print("The minimum value entered is: " . min($totalValidNumbers) . "</br>");
        print("The total of all the valid numbers counted together is: ".$total . "</br>");
        print("The average of all the valid numbers is: ". ($total / count($totalValidNumbers)). "</br>");

        print("<br>" . "<br>" );

        foreach ($invalid_values as $invalid){
          print("Invalid value: " . $invalid ."<br>");
        }
        ?>
  </div>
